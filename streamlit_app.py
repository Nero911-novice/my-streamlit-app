...
with tabs[0]:
    ...
    # Получаем верхнюю границу оси Y
    ymax = ax.get_ylim()[1]

    # Аннотации с фиксированным смещением в пунктах
    # 1σ — опускаем подписку ниже стрелки
    ax.annotate(
        "68% значений\n(±1σ)",
        xy=(mu, ymax * 0.9),
        xycoords='data',
        xytext=(0, -40),                 # вниз на 40 пунктов
    ...
    ax.annotate(
        "95% значений\n(±2σ)",
        xy=(mu - 2 * sigma, ymax * 0.6),
        xycoords='data',
        xytext=(-40, -10),               # влево и вниз
        textcoords='offset points',
        ha='left', va='top',
        arrowprops=dict(arrowstyle='->', color='black'),
        fontsize=9
    ...
    ax.annotate(
        "Почти все значения\nв пределах ±3σ",
        xy=(mu + 2.5 * sigma, ymax * 0.6),
        xycoords='data',
        xytext=(40, -10),                # вправо и вниз
        textcoords='offset points',
        ha='right', va='top',
        arrowprops=dict(arrowstyle='->', color='black'),
        fontsize=9
...
with tabs[2]:
    ...
    ax3.axhline(expected, color='r', linestyle='--',
                label=f"Теоретическое среднее ({expected:.2f})")
